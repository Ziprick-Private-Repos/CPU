...
module Control(input wire clk, input wire rstIn,
    ...
    .divDone(aluDivDone), .opcode(instruction[3:0]), 
    .regA(r1Out), .regB(tmpReg), .accumulator(accumOut), .remainder(remainder),
    .greaterFlag(greaterFlagBus), .zeroFlag(zeroFlagBus), .eqFlag(eqFlagBus), 
